...
from Model import TweetModel, TweetModelSentiment, TweetModel2
...
def train(args, train_loader, model, device, optimizer,scheduler, epoch, f, max_seq_len):
...
def valid(args, valid_loader, model, device, tokenizer, epoch, f, max_seq_len):
    ...
    with torch.no_grad():
        ...
        for step, d in enumerate(t):
            ...
            print_acc1 = xm.mesh_reduce("vacc1_reduce", accuracies1.avg, reduce_fn)
            print_acc2 = xm.mesh_reduce("vacc2_reduce", accuracies2.avg, reduce_fn)

            t.set_description(f"Eval E:{epoch+1} - Loss:{print_loss:0.2f} - Jac:{print_jac:0.2f} - acc1:{print_acc1:0.2f} - acc2:{print_acc2:0.2f}")
...
def main():
    ...
    MX = TweetModel2(model_path, config)
    ...
    #t_total = num_train_dpoints // args.gradient_accumulation_steps * args.epochs
    ...
    def run():
        ...
        for epoch in range(args.epochs):
            ...
            val_loss = xm.mesh_reduce("valid_loss_reduce", valid_loss, reduce_fn)

            xm.master_print(f"**** Epoch {epoch+1} **==>** Jaccard = {jac}")
            xm.master_print(f"**** Epoch {epoch+1} **==>** valid_loss = {val_loss}")
